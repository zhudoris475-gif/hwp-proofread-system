# -*- coding: utf-8 -*-
import os, sys, io, json, time, shutil, re, stat, struct, subprocess, zlib
...
OLLAMA_DOT_BATCH_SIZE = 8
OLLAMA_DOT_NUM_PREDICT_PER_ITEM = 80
OLLAMA_DOT_NUM_PREDICT_MIN = 160
OLLAMA_DOT_NUM_PREDICT_MAX = 600
OLLAMA_DOT_SINGLE_NUM_PREDICT = 180
OLLAMA_QUOTE_NUM_PREDICT = 400
...
def ensure_writable(path, log_fh=None):
    ...
    try:
        ...
        log(f"  쓰기 속성 해제 실패: {path} ({e})", log_fh)
    ...
    for attempt in range(max_retries + 1):
        try:
            if requests:
                ...
                r = _REQUESTS_SESSION.post(f"{OLLAMA_URL}/api/chat", json=payload, timeout=timeout)
                data = r.json()
...
def classify_dot_context_without_ollama(ctx):
    if not ctx:
        return None
    if re.search(r'[\u4e00-\u9fff]', ctx):
        return "keep", "한자 포함 문맥"
    if re.search(r'\d+\s*[·•・]\s*\d+', ctx):
        return "keep", "숫자/수량 문맥"
    if any(token and token in ctx for token in PROTECT_LIST):
        return "keep", "보호어 포함 문맥"
    return None
...
def collect_rule_based_dot_corrections(text, log_fh=None):
    ...
    for match in pattern.finditer(text):
        ...
        hit_count = text.count(orig)
        token_hist[token_count] = token_hist.get(token_count, 0) + 1
        log(f"  [가운데점-병렬 후보 {token_count}단어]: '{orig}' ({hit_count}회)", log_fh)
        if any(token and token in orig for token in PROTECT_LIST):
            log(f"  [가운데점-규칙 유지]: '{orig}' (보호어 예외, {hit_count}회)", log_fh)
            continue
        corr = orig.replace('·', ', ')
        if orig == corr:
            continue
        corrections.append((orig, corr))
        log(f"  [가운데점-규칙 교정]: '{orig}' -> '{corr}' ({token_count}단어 병렬 나열, {hit_count}회)", log_fh)
    if candidate_count:
...
def collect_dot_corrections_with_ollama(text, log_fh=None, prehandled_dot_items=None):
    ...
    seen_ollama_corrections = set(corrections)
    ollama_response_items = 0
    ollama_keep_items = 0
    rule_based_keep_contexts = []
    batch_size = max_ctx
    filtered_ollama_candidates = []
    for ctx in ollama_candidates:
        preclassified = classify_dot_context_without_ollama(ctx)
        if preclassified:
            action, reason = preclassified
            if action == "keep":
                rule_based_keep_contexts.append((ctx, reason))
                all_untouched_contexts.append(ctx)
                log(f"  [가운데점 규칙-유지]: '{ctx}' ({reason})", log_fh)
                continue
        filtered_ollama_candidates.append(ctx)
    ollama_candidates = filtered_ollama_candidates
    total_batches = (len(ollama_candidates) + batch_size - 1) // batch_size if ollama_candidates else 0
    if rule_based_keep_contexts:
        log(f"  [가운데점 규칙-유지 요약]: {len(rule_based_keep_contexts)}개", log_fh)

    def build_dot_prompt(dot_list):
        return f"""당신은 조선어 교정 전문가입니다.
...
JSON 배열만 출력하라."""
    ...
    for batch_index, start in enumerate(range(0, len(ollama_candidates), batch_size), 1):
        batch = ollama_candidates[start:start + batch_size]
        log(f"  [가운데점-Ollama 배치 {batch_index}/{total_batches}] 문맥 {len(batch)}개", log_fh)
        batch_success = False
        for attempt in range(3):
            dot_list = json.dumps(
                [{"id": i + 1, "context": ctx} for i, ctx in enumerate(batch)],
                ensure_ascii=False,
                indent=2,
            )
            dot_prompt = build_dot_prompt(dot_list)
            try:
                dot_num_predict = min(
                    OLLAMA_DOT_NUM_PREDICT_MAX,
                    max(OLLAMA_DOT_NUM_PREDICT_MIN, OLLAMA_DOT_NUM_PREDICT_PER_ITEM * len(batch))
                )
                response_json = post_ollama_chat({
                    "model": resolved_model,
                    "messages": [{"role": "user", "content": dot_prompt}],
                    "stream": False,
                    "options": {"num_predict": dot_num_predict, "temperature": 0}
                }, timeout=OLLAMA_DOT_TIMEOUT)
                resp_text = get_ollama_message_text(response_json)
                preview = resp_text[:300] if resp_text else "(빈 응답)"
                log(f"  [가운데점-Ollama 응답] (배치 {batch_index}, 시도 {attempt+1}, 문맥 {len(batch)}개): {preview}", log_fh)
                if not resp_text.strip():
                    raise RuntimeError("빈 응답")
                arr = parse_ollama_json_array(resp_text)
                ollama_response_items += len(arr)
                log(f"  [가운데점-Ollama 파싱] (배치 {batch_index}): 응답 {len(resp_text)}자 / JSON 항목 {len(arr)}개", log_fh)
                if not arr:
                ...
                for item in arr:
                    ...
                    if action == "convert" and orig and corr and orig != corr and orig in text:
                        if orig in rule_based_originals:
                            log(f"  [가운데점 교정-중복건너뜀]: '{orig}' (규칙 선반영)", log_fh)
                            continue
                        if (orig, corr) in seen_ollama_corrections:
                            log(f"  [가운데점 교정-중복건너뜀]: '{orig}' -> '{corr}'", log_fh)
                            continue
                        cnt = text.count(orig)
                        seen_ollama_corrections.add((orig, corr))
                        corrections.append((orig, corr))
                ...
                for ctx in untouched_contexts:
                    log(f"  [가운데점 유지-문맥]: '{ctx}' (Ollama 교정 항목 없음)", log_fh)
                all_untouched_contexts.extend(untouched_contexts)
                batch_success = True
                break
            except Exception as e:
                log(f"  가운데점 Ollama 오류 (배치 {batch_index}, 시도 {attempt+1}, 문맥 {len(batch)}개): {e}", log_fh)
                if attempt < 2:
                    time.sleep(3)
                else:
                    log(f"  가운데점 Ollama 배치 최종 실패: {batch_index}/{total_batches}", log_fh)
        if batch_success:
            continue

        log(f"  [가운데점-Ollama 개별 재시도] 배치 {batch_index} 문맥별 재질의 시작", log_fh)
        for local_idx, ctx in enumerate(batch, 1):
...
[{{"action":"keep","original":"","corrected":"","reason":"교정 불필요"}}]"""
            try:
                response_json = post_ollama_chat({
                    ...
                    "stream": False,
                    "options": {"num_predict": OLLAMA_DOT_SINGLE_NUM_PREDICT, "temperature": 0}
                }, timeout=OLLAMA_DOT_TIMEOUT)
                resp_text = get_ollama_message_text(response_json)
                arr = parse_ollama_json_array(resp_text)
                ollama_response_items += len(arr)
                log(f"  [가운데점-Ollama 개별 응답 {batch_index}-{local_idx}]: 응답 {len(resp_text)}자 / JSON 항목 {len(arr)}개", log_fh)
                matched = False
                for item in arr:
                    if not isinstance(item, dict):
                        continue
                ...
                all_untouched_contexts.append(ctx)
                log(f"  [가운데점 개별 오류 {batch_index}-{local_idx}]: {e}", log_fh)
    ...
    log(
        ...
        f"중한 선반영 제외 {len(prehandled_contexts)}개 / 규칙 선반영 {len(rule_based_corrections)}개 / "
        f"규칙 유지 {len(rule_based_keep_contexts)}개 / Ollama 대상 {len(ollama_candidates)}개 / Ollama 응답 {ollama_response_items}개 / "
        f"Ollama 유지 {ollama_keep_items}개 / 실제 교정 {len(corrections)}개 / 무응답 유지 {len(all_untouched_contexts)}개",
        log_fh,
    )
    return corrections
...
def classify_quote_without_ollama(q):
    convert, reason = should_convert_double_to_single(q)
    if convert and reason in {"중문 고정 명사", "중문 단어/구", "중문 숫자형"}:
        return "convert", reason
    if not convert:
        if reason == "대화형 문장" or reason.startswith("30자 초과") or reason in {"중한혼합", "기타 어구", "한글 어구(길이초과)", "빈 내용"}:
            return "keep", reason
    return None
...
def collect_quote_corrections(text, log_fh=None):
    ...
    for i, (orig, meta) in enumerate(quote_items, 1):
        ...
        preclassified = classify_quote_without_ollama(meta["quoted"])
        if preclassified:
            preclassified_items[orig] = preclassified
        else:
            ollama_quote_items.append((orig, meta))
        if i <= 50:
    ...
    rule_convert_count = 0
    rule_keep_count = 0
    for orig, meta in quote_items:
        preclassified = preclassified_items.get(orig)
        ...
        if action == "convert":
            corr = f"{LSQ}{q}{RSQ}"
            corrections.append((orig, corr))
            rule_convert_count += 1
            log(f"  [따옴표 규칙-교정]: '{orig}' -> '{corr}' ({reason}, {count}회)", log_fh)
        else:
            rule_keep_count += 1
            log(f"  [따옴표 규칙-유지]: '{orig}' ({reason}, {count}회)", log_fh)
    if preclassified_items:
        log(
            f"  [따옴표 규칙-선반영] 교정 {rule_convert_count}개 / 유지 {rule_keep_count}개 / "
            f"Ollama 대상 {len(ollama_quote_items)}개",
            log_fh,
        )
    if not ollama_quote_items:
        log(
            f"  [따옴표-상세요약] 후보 {len(quote_items)}개 / 규칙 교정 {rule_convert_count}개 / "
            f"규칙 유지 {rule_keep_count}개 / Ollama 대상 0개",
            log_fh,
        )
...
[후보 목록]
{chr(10).join(
    f"  {i}. {orig} | 출현 {meta['count']}회 | 사전힌트: "
    f"{'변환권고' if should_convert_double_to_single(meta['quoted'])[0] else '유지권고'} "
    f"({should_convert_double_to_single(meta['quoted'])[1]})"
    for i, (orig, meta) in enumerate(ollama_quote_items, 1)
)}
...
JSON 배열만 출력하라."""
    ...
    try:
        response_json = post_ollama_chat({
            ...
            "stream": False,
            "options": {"num_predict": OLLAMA_QUOTE_NUM_PREDICT, "temperature": 0}
        ...
        ollama_hit_map = {orig: meta for orig, meta in ollama_quote_items}
        responded = set()
        keep_count = 0
        for item in arr:
            if not isinstance(item, dict):
            ...
            if not orig or orig not in ollama_hit_map:
                continue
            responded.add(orig)
            q = ollama_hit_map[orig]["quoted"]
            count = ollama_hit_map[orig]["count"]
            if action == "convert":
                corr = f"{LSQ}{q}{RSQ}"
                corrections.append((orig, corr))
        ...
        no_response_items = []
        for orig, meta in ollama_quote_items:
        ...
        log(
            f"  [따옴표-상세요약] 후보 {len(quote_items)}개 / 규칙 교정 {rule_convert_count}개 / "
            f"규칙 유지 {rule_keep_count}개 / Ollama 대상 {len(ollama_quote_items)}개 / "
            f"Ollama 응답 {len(arr)}개 / 총 교정 {len(corrections)}개 / Ollama 유지 {keep_count}개 / "
            f"응답없음 {len(no_response_items)}개",
            log_fh,
        )
    except Exception as e:
...
def process_hwp_file(filepath, txt_rules, regex_rules, china_rules, log_fh=None):
    ...
    temp_path = None
    try:
        ensure_writable(filepath, log_fh)
        ole = olefile.OleFileIO(filepath, write_mode=True)
    except PermissionError:
        temp_dir = os.path.join(os.environ.get('TEMP', 'C:\\Temp'), 'hwp_proofread')
        os.makedirs(temp_dir, exist_ok=True)
        temp_path = os.path.join(temp_dir, f"corrected_{os.path.basename(filepath)}")
        shutil.copy2(filepath, temp_path)
        ensure_writable(temp_path, log_fh)
        log(f"  원본 잠금 - 복사본으로 우회: {temp_path}", log_fh)
        ole = olefile.OleFileIO(temp_path, write_mode=True)
    except Exception as e:
        log(f"  OLE 열기 실패: {e}", log_fh)
        return 0, all_corrections

    try:
        streams = ole.listdir()
        body_streams = [s for s in streams if s and s[0] == "BodyText"]
        log(f"  BodyText 스트림 수: {len(body_streams)}", log_fh)
        ...
        for stream_path in body_streams:
            ...
            dec = decompress_chain(orig_data)
            if dec is None:
                log(f"  {stream_name}: 압축 해제 실패", log_fh)
                continue

            records = parse_records(dec)
            log(f"  {stream_name}: {len(records)}개 레코드, 압축 크기: {orig_size:,}바이트", log_fh)
            ...
            for rec in records:
                if rec["tag_id"] != 67:
                    continue
                try:
                    record_text = rec["payload"].decode("utf-16-le", errors="replace")
                except Exception:
                    continue
                ...
                for src, dst, rule_type, cnt in all_corrections:
                    if src in new_text:
                        replace_count = new_text.count(src)
                        new_text = new_text.replace(src, dst)
                        stream_changes += replace_count

                if new_text != record_text:
                    rec["payload"] = new_text.encode("utf-16-le")

            if stream_changes > 0:
                new_dec = serialize_records(records)
                new_body = compress_to_size(new_dec, orig_size)
                if new_body is None:
                    log(f"  {stream_name}: 압축 크기 초과로 저장 불가", log_fh)
                else:
                    try:
                        ole.write_stream(stream_name, new_body)
                        total_applied += stream_changes
                        log(f"  {stream_name} 변경: {stream_changes}건 적용", log_fh)
                    except Exception as e:
                        log(f"  {stream_name} 쓰기 오류: {e}", log_fh)

    finally:
        ole.close()

    if temp_path and total_applied > 0:
        try:
            shutil.copy2(temp_path, filepath)
            log(f"  원본 교체 완료: {filepath}", log_fh)
        except Exception as e:
            log(f"  원본 교체 실패: {e} - 결과 파일: {temp_path}", log_fh)
