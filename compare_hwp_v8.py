...
def content_level_compare(orig_clean, corr_clean, section_key):
    ...
    print(f"    1차 삭제: {len(raw_del_sentences)}개 | 1차 추가: {len(raw_add_sentences)}개")
    ...
    print(f"    ★ 진짜 삭제: {len(truly_deleted)}개 | 이동/수정: {len(moved_or_modified)}개")
    print(f"    ★ 진짜 추가: {len(truly_added)}개")
    ...
    print(f"    확정 삭제 단어(원본만): {len(confirmed_del_words)}개 → 진짜: {len(truly_confirmed_del)}개")
    ...
    print(f"    한국어 문장 삭제: {len(del_kr_sents)}개 → 진짜: {len(truly_del_kr)}개")
...
def compare_section(orig_path, corr_path, section_label, section_key):
    ...
    print("\n[5/7] 콘텐츠 수준 비교 (중국어 문장/단어 + 유사도 검증)...")
    ...
    print(f"  삭제: {len(del_kr)}개 | 추가: {len(add_kr)}개")
...
def write_log(result, log_path):
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write("=" * 120 + "\n")
        f.write("HWP 파일 상세 비교 로그 v8 — 검증 하이브리드 비교 시스템\n")
        f.write(f"대중한사전(大中朝) {result['section_label']}\n")
        f.write("=" * 120 + "\n")
        f.write(f"생성일시: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"방식: ①표제어매칭 ②중국어문장비교 ③유사도검증(0.8)으로 이동/수정 필터링\n")
        f.write("=" * 120 + "\n\n")
        ...
        f.write("[2] ★★★ 중국어 문장 비교 (유사도 검증 적용) ★★★\n")
        f.write("-" * 80 + "\n")
        f.write(f"  원본 중국어 문장: {cr['orig_chinese_sentences']}개\n")
        f.write(f"  교정본 중국어 문장: {cr['corr_chinese_sentences']}개\n")
        f.write(f"  1차 삭제 (단순비교): {cr['raw_deleted_chinese_sentences']}개\n")
        f.write(f"  이동/수정됨 (유사도≥0.8): {len(cr['moved_or_modified_sentences'])}개\n")
        f.write(f"  ★ 진짜 삭제됨: {len(cr['truly_deleted_chinese_sentences'])}개\n")
        f.write(f"  ★ 진짜 추가됨: {len(cr['truly_added_chinese_sentences'])}개\n\n")

        if cr['truly_deleted_chinese_sentences']:
            f.write("\n[2-1] ★★★ 진짜 삭제된 중국어 문장 ★★★\n")
            f.write("-" * 80 + "\n\n")
        ...
        if cr['moved_or_modified_sentences']:
            f.write(f"\n[2-2] 이동/수정된 중국어 문장 ({len(cr['moved_or_modified_sentences'])}개, 첫 30개)\n")
            f.write("-" * 80 + "\n\n")
            for s in cr['moved_or_modified_sentences'][:30]:
                f.write(f"  {s}\n")

        f.write("\n[3] 중국어 단어 비교 (유사도 검증)\n")
        f.write("-" * 80 + "\n")
        f.write(f"  원본 단어: {cr['orig_chinese_words']}개 | 교정본: {cr['corr_chinese_words']}개\n")
        ...
        f.write("\n[4] 한국어 문장 비교\n")
        f.write("-" * 80 + "\n")
        f.write(f"  원본: {cr['orig_korean_sentences']}개 | 교정본: {cr['corr_korean_sentences']}개\n")
        f.write(f"  1차 삭제: {len(cr['deleted_korean_sentences'])}개\n")
        f.write(f"  ★ 진짜 삭제: {len(cr['truly_deleted_korean_sentences'])}개\n\n")

        if cr.get('truly_deleted_korean_sentence_context'):
            f.write("\n[4-1] 진짜 삭제된 한국어 문장 (최대 300개)\n")
            f.write("-" * 80 + "\n\n")
            for item in cr['truly_deleted_korean_sentence_context']:
                s = item['sentence']
                f.write(f"  {s[:200]}\n\n")
        ...
        if hr:
            f.write("\n[5] 표제어 기반 비교\n")
            f.write("-" * 80 + "\n")
            f.write(f"  완전삭제: {len(hr['deleted_entries'])}개 | 추가: {len(hr['added_entries'])}개\n")
            ...
            if hr['chinese_deleted']:
                ...
                for i, d in enumerate(hr['chinese_deleted'], 1):
                    ...
                    f.write(f"     확정삭제: {', '.join(d.get('real_deleted_chinese', [])[:10])}\n")
        ...
        if cr['truly_deleted_chinese_sentences']:
            f.write(f"  [긴급] 진짜 삭제된 중국어 문장: {len(cr['truly_deleted_chinese_sentences'])}개\n")
            for item in cr['deleted_chinese_sentence_context'][:30]:
                f.write(f"    ▶ {item['sentence']}\n")
            f.write("\n")
        else:
            f.write("  ✓ 중국어 문장 삭제 없음 — 교정본에 모든 중국어 내용이 보존됨\n\n")

        if cr['truly_confirmed_deleted_words']:
            f.write(f"  [확인] 진짜 삭제된 중국어 단어: {len(cr['truly_confirmed_deleted_words'])}개\n")
            for item in cr['deleted_chinese_word_context'][:30]:
                f.write(f"    ▶ {item['word']}\n")
            f.write("\n")
        else:
            f.write("  ✓ 확정 중국어 단어 삭제 없음\n\n")
...
def main():
    ...
    for section_key, info in SECTIONS.items():
        ...
        log_path = os.path.join(LOG_DIR, f"hwp_comparison_log_v8_{section_key}_{timestamp}.txt")
        write_log(result, log_path)

        cr = result['content_result']
        print(f"\n  로그: {log_path}")
        print(f"  ★ 최종 결과:")
        print(f"    진짜 삭제된 중국어 문장: {len(cr['truly_deleted_chinese_sentences'])}개")
        print(f"    이동/수정된 문장: {len(cr['moved_or_modified_sentences'])}개")
        print(f"    진짜 삭제된 중국어 단어: {len(cr['truly_confirmed_deleted_words'])}개")
        print(f"    진짜 삭제된 한국어 문장: {len(cr['truly_deleted_korean_sentences'])}개")
