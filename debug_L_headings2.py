...
print(f"원본 표제어 수: {len(orig_headings)}")
print(f"교정본 표제어 수: {len(corr_headings)}")

print("\n=== 원본 표제어 첫 50개 ===")
for i, h in enumerate(orig_headings[:50]):
    print(f"  [{i}] 【{h}】")

print("\n=== 교정본 표제어 첫 50개 ===")
for i, h in enumerate(corr_headings[:50]):
    print(f"  [{i}] 【{h}】")
...
dupes = {k: v for k, v in orig_counter.items() if v > 1}
if dupes:
    print(f"\n원본 중복 표제어: {len(dupes)}개")
    for k, v in list(dupes.items())[:10]:
        print(f"  【{k}】×{v}")

corr_counter = Counter(corr_headings)
dupes2 = {k: v for k, v in corr_counter.items() if v > 1}
if dupes2:
    print(f"\n교정본 중복 표제어: {len(dupes2)}개")
    for k, v in list(dupes2.items())[:10]:
        print(f"  【{k}】×{v}")
