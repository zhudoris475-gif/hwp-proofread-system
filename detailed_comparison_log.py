...
def detailed_comparison(pr, label, text_orig, text_final, rules):
    pr(f"\n{'=' * 80}")
...
def check_p_integrity(pr, text, rules):
    pr(f"\n{'=' * 80}")
    pr(f"  [P파일] 한자 무결성 검사 상세로그")
    pr(f"{'=' * 80}")
    ...
    pr(f"\n--- P파일 '·뒤한자' 세그먼트 전체 목록 ---")
    dot_cjk_counter = Counter(re.findall(r'([가-힣]+(?:성|시|자치구|행정구|도|구|현|진|산|호|강|하|부|주|족))·([^\s·\)]*[一-龥][^\s·\)]*)', text))
    for (kor, cjk), cnt in dot_cjk_counter.most_common():
        pr(f"    {kor}·{cjk} ({cnt}회)")

    pr(f"\n--- P파일 한자 출현 빈도 상위 30개 ---")
    pr(f"{'문자':<4} {'코드':<10} {'출현수':>8} {'규칙관련':<8}")
    pr("-" * 40)
    for ch, cnt in cjk_counter.most_common(30):
        rule_rel = "✅" if ch in rules_src_set else "-"
        pr(f"  {ch}  U+{ord(ch):04X}  {cnt:>8} {rule_rel}")
...
out_path = rf"c:\Users\doris\AppData\Local\Temp\hwp_logs\JLP_detailed_comparison_log_{ts}.txt"
...
with open(out_path, "w", encoding="utf-8") as OUT:
    ...
    pr("\n[5/6] J/L/O 대비과정 상세 비교...")
    j_result = detailed_comparison(pr, "J", t_jo, t_jf, all_rules)
    l_result = detailed_comparison(pr, "L", t_lo, t_lf, all_rules)
    o_result = detailed_comparison(pr, "O", t_oo, t_of, all_rules)

    pr("\n[6/6] P파일 한자 무결성 검사...")
    p_check = check_p_integrity(pr, t_p, all_rules)

    pr(f"\n{'=' * 80}")
    pr(f"  J/L/P/O 대비과정 종합 결론")
    pr(f"{'=' * 80}")

    pr(f"\n■ 파일별 한자 변경 요약")
    pr(f"{'파일':<6} {'원본한자':>10} {'최종한자':>10} {'차이':>8} {'정상':>6} {'비정상':>6} {'확인':>6}")
    pr("-" * 60)
    for lbl, r in [("J", j_result), ("L", l_result), ("O", o_result)]:
        pr(f"  {lbl:<4} {r['orig_cjk']:>10,} {r['final_cjk']:>10,} {r['final_cjk']-r['orig_cjk']:>+8} {r['normal_count']:>6} {r['abnormal_count']:>6} {r['need_check_count']:>6}")

    total_normal = sum(r['normal_count'] for r in [j_result, l_result, o_result])
    total_abnormal = sum(r['abnormal_count'] for r in [j_result, l_result, o_result])
    total_need_check = sum(r['need_check_count'] for r in [j_result, l_result, o_result])
    ...
    checks = [
        ...
        ("O파일 비정상 변경 없음", o_result['abnormal_count'] == 0, f"{o_result['abnormal_count']}건"),
        ...
        ("O파일 한자 출현수 동일", o_result['orig_cjk'] == o_result['final_cjk'],
         f"{o_result['orig_cjk']:,}→{o_result['final_cjk']:,}"),
        ("P파일 한자 존재", p_check['cjk_total'] > 0, f"{p_check['cjk_total']:,}회"),
        ("P파일 '·뒤한자' 존재", p_check['dot_cjk_count'] > 0, f"{p_check['dot_cjk_count']}건"),
        ("P파일 '한국성·중국省' 패턴 존재", p_check['sheng_pattern_count'] > 0, f"{p_check['sheng_pattern_count']}건"),
    ]
    ...
    if total_abnormal > 0:
        ...
        pr(f"      - 원본에서 삭제된 한자 세그먼트 위치 추출")
        pr(f"      - 교정본에 '(중국지명)' 형식으로 재삽입")
        pr(f"  [2] 교정 스크립트 '·' 처리 로직 수정")
        pr(f"      - 가운뎃점→괄호 변환 시 중국어 원문 보존")
