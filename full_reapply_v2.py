...
print(f"1. 원본 복원: {os.path.getsize(hwp_path):,} bytes")
...
print("\n3. 교정 적용:")
applied = 0
for i, (src, dst, rule_type) in enumerate(corrections):
    try:
        ...
        if result:
            applied += 1
            print(f"  [{i+1}/{len(corrections)}] [OK] '{src}' -> '{dst}'")
        else:
            print(f"  [{i+1}/{len(corrections)}] [SKIP] '{src}' -> '{dst}'")
        time.sleep(0.5)
    except Exception as e:
        print(f"  [{i+1}/{len(corrections)}] [ERROR] '{src}': {e}")
...
if all_ok and save_ok:
    ...
    print("  저장 성공, 일부 교정 미적용")
else:
    print("  저장 실패 - 추가 조치 필요")
print(f"{'='*60}")
