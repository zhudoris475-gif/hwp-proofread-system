...
try:
    ...
    applied = skipped = 0
    log_lines = [f"HWP COM FindReplace 교정", f"시간: {time.strftime('%Y-%m-%d %H:%M:%S')}", "="*50]
    ...
    report = os.path.join(REPORT_DIR, f"O_COM_FindReplace_{ts}.txt")
    with open(report, 'w', encoding='utf-8') as f:
        f.write('\n'.join(log_lines))
    
    print(f"\n{'='*50}")
    print(f"✅ COM FindReplace 완료!")
