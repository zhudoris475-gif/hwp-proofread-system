...
def apply_corrections_via_com(filepath, all_corrections, log_fh=None):
    ...
    try:
        ...
        quote_chars_done = False
        for src, dst, rule_type, cnt in all_corrections:
            if cnt <= 0:
                continue
            if rule_type == "따옴표" and not quote_chars_done:
                quote_chars_done = True
                for qsrc, qdst in [("\u201c", "\u2018"), ("\u201d", "\u2019")]:
                    try:
                        ...
                        if result:
                            qcnt = text_len if text_len else cnt
                            total_applied += cnt
                            log(f"  [COM-따옴표문자]: '{qsrc}' -> '{qdst}' (적용)", log_fh)
                        else:
                            log(f"  [COM-따옴표문자 SKIP]: '{qsrc}' -> '{qdst}'", log_fh)
                        time.sleep(0.5)
                    except Exception as e:
                        log(f"  [COM-따옴표문자 실패]: '{qsrc}' -> '{qdst}' ({e})", log_fh)
                continue
            try:
                hwp.HAction.GetDefault("AllReplace", hwp.HParameterSet.HFindReplace.HSet)
                ...
                if result:
                    ...
                    log(f"  [COM-{rule_type} SKIP]: '{src}' -> '{dst}' (검색안됨)", log_fh)
                time.sleep(0.3)
            except Exception as e:
...
def save_report(filepath, all_corrections, total_applied, log_fh=None):
    ...
    try:
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(f"{'=' * 60}\n")
            f.write(f"HWP 교정 결과 리포트 v17.1 (바이너리 + Ollama + 따옴표FindType수정)\n")
            f.write(f"{'=' * 60}\n")
...
def main():
    ...
    if not args:
        print("=" * 55)
        print("  HWP 교정 v17.1 (바이너리 + Ollama + 따옴표FindType수정)")
    ...
    with open(LOG_FILE, 'a', encoding='utf-8') as log_fh:
        log(f"\n\n{'#' * 60}", log_fh)
        log(f"HWP 교정 v17.1: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", log_fh)
