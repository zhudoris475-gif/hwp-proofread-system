...
DEPENDENT_NOUNS = {
    '수', '뿐', '적', '지', '바', '것', '데', '중', '상', '앞', '등', '때',
    '사이', '이하', '이상',
}

DEPENDENT_NOUN_PHRASES = {
    '뿔만', '뿐만', '것 같은', '는 데', '따위',
}

SPACING_RULES = {
    'dependent_noun_separate': {
        'description': '의존 명사는 앞말에서 띄어 씀',
        'examples': ['할 수', '있을 뿐', '하는 것', '있는 데', '그 중', '이 상', '그 앞', '그 등', '그 때', '그 사이'],
        'pattern': r'([가-힣]+)({})'.format('|'.join(sorted(DEPENDENT_NOUNS, key=len, reverse=True))),
        'replacement': r'\1 \2',
    },
    'dependent_noun_phrase_separate': {
        'description': '의존 명사 구는 앞말에서 띄어 씀',
        'examples': ['할 뿐만', '하는 것 같은', '척하다', '고 하다', '따위'],
        'pattern': r'([가-힣]+)({})'.format('|'.join(sorted(DEPENDENT_NOUN_PHRASES, key=len, reverse=True))),
        'replacement': r'\1 \2',
    },
    'particle_attach': {
        'description': '조사는 앞말에 붙여 씀',
        'examples': ['사이에', '것이', '수가', '때문에'],
        'particles': ['이', '가', '을', '를', '은', '는', '도', '만', '까지', '부터', '까지', '에', '에서', '으로', '로', '와', '과', '하고', '의', '께', '께서', '보다', '처럼', '만큼', '밖에', '뿐', '마저', '조차'],
    },
}
...
PROVINCE_ABBREV = {
    ...
    '吉林省': '吉林', '辽宁省': '辽宁', '浙江省': '浙江',
    '安徽省': '安徽', '福建省': '福建', '江西省': '江西',
    '江苏省': '江苏', '台湾省': '台湾', '海南省': '海南',
}
