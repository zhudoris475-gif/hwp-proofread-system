...
GEC_MODEL_NAME = "Soyoung97/gec_kr"
TYPOS_MODEL_NAME = "j5ng/et5-typos-corrector"
...
class AIModelManager:
    def __init__(self, logger: DetailedLogger):
        ...
        self.typos_model = None
        self.typos_tokenizer = None
    ...
    def load_models(self):
        ...
        try:
            ...
            self.logger.log("  GEC 모델 로드 중: " + GEC_MODEL_NAME)
            ...
            self.gec_model.eval()
            self.logger.log("  GEC 모델 로드 완료")
            
            self.logger.log("  오타 교정 모델 로드 중: " + TYPOS_MODEL_NAME)
            self.typos_tokenizer = AutoTokenizer.from_pretrained(TYPOS_MODEL_NAME)
            self.typos_model = AutoModelForSeq2SeqLM.from_pretrained(TYPOS_MODEL_NAME)
            self.typos_model.eval()
            self.logger.log("  오타 교정 모델 로드 완료")
    ...
    def correct_grammar(self, text: str) -> Tuple[str, bool]:
        if not self.models_loaded or not self.gec_model:
            return text, False
            
        try:
            raw_input_ids = self.gec_tokenizer.encode(text)
            input_ids = [self.gec_tokenizer.bos_token_id] + raw_input_ids + [self.gec_tokenizer.eos_token_id]
            
            with torch.no_grad():
                corrected_ids = self.gec_model.generate(
            ...
            corrected = self.gec_tokenizer.decode(corrected_ids.squeeze().tolist(), skip_special_tokens=True)
            return corrected, corrected != text
        except Exception as e:
            self.logger.log_error("GEC", str(e))
            return text, False
            
    def correct_typos(self, text: str) -> Tuple[str, bool]:
        if not self.models_loaded or not self.typos_model:
            return text, False
            
        try:
            inputs = self.typos_tokenizer(text, return_tensors="pt", padding=True)
            
            with torch.no_grad():
                outputs = self.typos_model.generate(
                    inputs.input_ids,
                    max_length=256,
                    num_beams=4,
                    early_stopping=True
            ...
            corrected = self.typos_tokenizer.decode(outputs[0], skip_special_tokens=True)
            return corrected, corrected != text
        except Exception as e:
            self.logger.log_error("오타 교정", str(e))
            return text, False
...
def apply_quote_rules(text: str, logger: DetailedLogger) -> List[Tuple[str, str]]:
    ...
    single_quote_patterns = [
        (re.compile(r'(?<=[가-힣])"(?=[가-힣])'), '"', "단일 따옴표를 쌍따옴표로"),
        (re.compile(r'(?<=[가-힣])\'(?=[가-힣])'), "'", "단일 따옴표를 쌍따옴표로"),
    ]

    for pattern, replacement, reason in single_quote_patterns:
        for m in pattern.finditer(text):
            ...
            logger.log_correction("따옴표", orig, replacement, f"{reason} (문맥: {ctx})")
...
def proofread_file(filepath: str, rules: List[Tuple[str, str]], china_rules: List[Tuple[str, str]],
    ...
    if stages in ["all", "4"]:
        ...
        all_corrections.extend(stage_corrections)
        text = apply_corrections(text, stage_corrections, logger)
        logger.log(f"  4단계 결과: {len(stage_corrections)}개 교정")
