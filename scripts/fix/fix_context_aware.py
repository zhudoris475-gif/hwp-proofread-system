...
def generate_context_rules(text):
    ...
    # 추가 데 의존명사 패턴 (관형사형 + 데)
    de_dep_extra = [
        (r'쓸데(가|는|로|서|에|를|이|도|만)', '쓸 데\\1'),
        (r'이를데(가|로|서|에|를|이|도|만)', '이를 데\\1'),
        (r'비할데(가|로|서|에|를|이|도|만)', '비할 데\\1'),
        (r'갈데(가|로|서|에|를|이|도|만)', '갈 데\\1'),
        (r'먹을데(가|로|서|에|를|이|도|만)', '먹을 데\\1'),
        (r'볼데(가|로|서|에|를|이|도|만)', '볼 데\\1'),
        (r'알데(가|로|서|에|를|이|도|만)', '알 데\\1'),
        (r'살데(가|로|서|에|를|이|도|만)', '살 데\\1'),
        (r'놀데(가|로|서|에|를|이|도|만)', '놀 데\\1'),
        (r'다닐데(가|로|서|에|를|이|도|만)', '다닐 데\\1'),
        (r'지낼데(가|로|서|에|를|이|도|만)', '지낼 데\\1'),
        (r'기대할데(가|로|서|에|를|이|도|만)', '기대할 데\\1'),
        (r'쉴데(가|로|서|에|를|이|도|만)', '쉴 데\\1'),
        (r'묵을데(가|로|서|에|를|이|도|만)', '묵을 데\\1'),
        (r'배울데(가|로|서|에|를|이|도|만)', '배울 데\\1'),
    ]
    ...
    ge_dep_stems = ['할', '될', '갈', '볼', '알', '쓸', '칠', '먹을', '만들', '줄',
                    '놓을', '다닐', '살', '열', '팔', '뽑을', '고를', '고칠', '넣을',
                    '지을', '지킬', '따를', '이길', '느낄', '바꿀', '뗄', '그릴',
                    '누릴', '맡을', '외울', '일으킬', '읽을', '찾을', '닦을', '뛸',
                    '부를', '지낼', '빚을', '깎을', '걸을', '받을', '탈']
