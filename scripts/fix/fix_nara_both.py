...
def fix_nara(filepath):
    ...
    if not needed:
        print(f"  [{fname}] 나라→조 미적용 없음 - 스킵")
        return True

    print(f"  [{fname}] 나라→조 미적용: {len(needed)}개")
...
def main():
    ...
    for fpath in HWP_FILES:
        ...
        for src, dst in NARA_RULES:
            cnt = text.count(src)
            if cnt > 0:
                print(f"  [{fname}] 아직 남음: '{src}' {cnt}건")
                remaining += cnt
        if remaining == 0:
            print(f"  [{fname}] ✅ 나라→조 모두 완료!")
