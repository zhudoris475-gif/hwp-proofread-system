...
def main():
    for label, fpath in FILES.items():
        ...
        print(f"[{label}] 텍스트 길이: {len(text):,}자")
        ...
        try:
            ...
            try:
                hwp.HAction.GetDefault("Save", hwp.HParameterSet.HFileOpenSave.HSet)
                result = hwp.HAction.Execute("Save", hwp.HParameterSet.HFileOpenSave.HSet)
                if result:
                    print(f"  저장 성공")
                else:
                    print(f"  저장 실패(Action)")
                    try:
                        hwp.SaveAs(fpath, "HWP", "")
                        print(f"  SaveAs로 저장 성공")
