...
def extract_and_correct_hwp(hwp_file_path):
    ...
    try:
        ...
        # Note: HwpBasic.HwpObject is not registered, so we'll use a different approach
        # This script will use Hwp.exe to open the file and apply corrections

        powershell_script = f"""
        $ErrorActionPreference = "Stop"

        try {{
            # Open HWP file using Hwp.exe
